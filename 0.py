...
def _token_set(s1, s2, partial=True, force_ascii=True, do_full_process=True):
    ...
    if partial:
        ...
        ratio_func = ratio
    pairwise = [ratio_func(sorted_sect, combined_1to2), ratio_func(sorted_sect, combined_2to1), ratio_func(combined_1to2, combined_2to1)]
    return max(pairwise)
...
def WRatio(s1, s2, force_ascii=True, do_full_process=True):
    ...
    **Steps in the order they occur**
    ...
    if try_partial:
        ...
        ptsor = partial_token_sort_ratio(p1, p2, do_full_process=False) * unbase_scale * partial_scale
        ptser = partial_token_set_ratio(p1, p2, do_full_process=False) * unbase_scale * partial_scale
...
def extractWithoutOrder(query, choices, processor=default_processor, scorer=default_scorer, score_cutoff=0):
    ...
    if len(processed_query) == 0:
        logging.warning(u"Applied processor reduces input query to empty string, all comparisons will have score 0. [Query: '{0}']".format(query))
    if scorer in [WRatio, QRatio, token_set_ratio, token_sort_ratio, partial_token_set_ratio, partial_token_sort_ratio, UWRatio, UQRatio] and processor == full_process:
        processor = no_process
    ...
    elif scorer in [WRatio, QRatio, token_set_ratio, token_sort_ratio, partial_token_set_ratio, partial_token_sort_ratio]:
...
def extract(query, choices, processor=default_processor, scorer=default_scorer, limit=5):
    ...
    return heapq.nlargest(limit, sl, key=lambda i: i[1]) if limit is not None else sorted(sl, key=lambda i: i[1], reverse=True)
...
def extractBests(query, choices, processor=default_processor, scorer=default_scorer, score_cutoff=0, limit=5):
    ...
    return heapq.nlargest(limit, best_list, key=lambda i: i[1]) if limit is not None else sorted(best_list, key=lambda i: i[1], reverse=True)
