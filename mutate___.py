...
class NodeFinder(ast.NodeTransformer):
    ...
    def visit_FunctionDef(self, node):
        if node.name == self.func_to_mutate:
            mutator = MutationChamber(mutants_so_far=self.mutants_so_far, func_tree_height=height(node), dont_mutate_until=self.dont_mutate_until, max_mutations=self.max_mutations)
...
class MutationChamber(ast.NodeTransformer):
    def __init__(self, mutants_so_far,func_tree_height, dont_mutate_until: int, max_mutations: int = 1, ):
        self.num_mutations = 0
        self.dont_mutate_until = dont_mutate_until
        self.nodes_so_far = 0
        self.max_mutations = max_mutations
        self.mutants_so_far = mutants_so_far
        # We are going to try some shit.
        # First we will randomly decide upon each MutationChamber.__init__() execution
        # Whether to execute a shallow or deep node. We will do this by calculating the height of the tree, 
        # then randomly deciding at which height we will perform a mutation.
        self.mutation_height = random.randint(func_tree_height//2, func_tree_height - 1)
        if(random.choice([True,False])):
            self.mutation_height = r
        else:
            self.mutation_height = random.randint(1, func_tree_height//2)
    ...
    def shouldMutate(self, node):
        return self.num_mutations < self.max_mutations and height(node) - self.mutation_height == 0
    ...
    def visit_BinOp(self, node):
        if(self.shouldMutate(node)):
            ...
            if isinstance(node.op, ast.Add):
                ...
                # I dont find this useful, Im not convinced someone would actually do this by accident.
    ...
    while(mutants_so_far < num_mutants):
        for(func_name, call_count) in treeData["call_count"]:
            ...
            if(True):
                ...
                if(True): # TODO: the commented out shit above is from the good submit...
                    mutator = NodeFinder(func_name, i, current_max_mutations, mutants_so_far)
